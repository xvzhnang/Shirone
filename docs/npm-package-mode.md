...
`src/integration/index.ts` resolves its location at config time
(`paths.ts` `detectThemeRepo`), which asks a single question — is the theme
building itself? — and derives everything else from the answer:

- **Source mode (in-repo)** — this repository also runs
  `integrations: [shirones()]`; the repo `astro.config.mjs` is a one-liner.
  The in-repo detection points `configDir`/`dataDir`/`contentDir` back at
  `src/` itself, so Astro's file-based routing still picks up `src/pages/`
  directly. The overlay plugin and the override registry are skipped (the
  theme's own files are the resolution targets), the source-only esbuild
  `drop`/`pure` build options are re-enabled for `astro build`, font subsets
  are written to the repo's historical `src/assets/fonts/.subset/` location,
  and Pagefind indexing stays with the repo's CLI build step (which honours
  `pagefind.yml`).
- **Package mode** — the user's `astro.config.mjs` contains only
  `integrations: [shirones()]`. The integration then registers the bundled
  integrations, builds the font declarations, installs the markdown processor,
  and injects every page in `src/pages/` with `injectRoute`. Subsets are
  emitted into `<projectRoot>/.shirones/fonts/` and the Pagefind index is
  generated by the integration's `astro:build:done` hook.

  A *linked* install (`pnpm link`, a pnpm workspace, `npm link`) counts as
  package mode. It used to fall between the two: the old detection paired an
  `isPluginMode` check on `import.meta.url` with the repository comparison, and
  a symlink resolves to its realpath, so the URL carried no `node_modules`
  segment and neither flag fired. That state combined package-mode directory
  defaults with source-mode behaviour — no routes injected, no `npx shirones
  init` hint — and produced a silently empty site.

## Architecture

```text
src/integration/
├── index.ts        integration entry — the single config driver for every mode
├── overlay.ts      override system (Vite resolveId; package mode only)
...
- There is a single Astro config entry point: `src/integration/index.ts`
  drives both this repository and user projects (the repo `astro.config.mjs`
  is just `integrations: [shirones()]`). The *option values* come from
  `src/config/integrationsConfig.ts`; the wiring — vite aliases,
  `svelte().preprocess`, `optimizeDeps` filtering, `vite.plugins`, how
  expressive-code's `themes` and `plugins` resolve — lives in `index.ts`,
  with per-mode branches where the layouts genuinely differ (in-repo skips
  the overlay plugin and the override registry; only `astro build` inside the
  repo re-enables esbuild `drop`/`pure`). Add option values to
  `integrationsConfig.ts` and wiring to `index.ts`; there is no second entry
  point left to forget.
